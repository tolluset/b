...
pygame.display.set_caption("영어-카타카나 단어 게임")
...
# Load custom fonts
jp_font_path = os.path.join("aseets", "fonts", "Noto_Sans_JP", "NotoSansJP-VariableFont_wght.ttf")
en_font_path = os.path.join("aseets", "fonts", "Noto_Sans", "NotoSans-VariableFont_wdth,wght.ttf")
...
def game_over_screen():
    ...
    for i, (english, japanese) in enumerate(all_words_list):
        ...
        # Skip if we're going to go off screen
        if column > 1 or y_pos > SCREEN_HEIGHT - 120:
            continue
...
def main():
    global lives, score, game_active, start_time, input_text, matched_words, missed_words, all_shown_words, game_time, ime_text
    ...
    while True:
            ...
            if game_active:
                if event.type == KEYDOWN:
                    if event.key == K_RETURN:
                        ...
                        input_to_check = input_text + ime_text
                        for word in falling_words[:]:
                            if input_to_check == word.japanese:
        ...
        if game_active:
            ...
            if current_time - last_spawn_time > spawn_interval:
                new_word = spawn_word(word_pairs)
                if new_word:
                    falling_words.append(new_word)
                    last_spawn_time = current_time
                    # Gradually decrease spawn interval to increase difficulty, but more slowly
                    spawn_interval = max(2.0, spawn_interval * 0.99)

            # Update falling words
            for word in falling_words[:]:
                word.update()
                if word.is_out_of_bounds():
